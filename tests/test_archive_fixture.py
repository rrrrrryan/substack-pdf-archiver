...
def test_archive_local_fixture_writes_pdf_metadata_and_downloads(
    ...
    assert result.metadata_path.name == f"{expected_stem}.archive.json"
    ...
    assert metadata["publication_name"] == "Example Publication"
    assert metadata["published_at"] == "2026-03-24T08:15:00+00:00"
    assert metadata["attachments"][0]["filename"] == "deck.pdf"
    assert metadata["debug_artifacts"]["cleaned_html"].endswith("cleaned.html")


def test_archive_explicit_output_keeps_path_and_applies_manual_publication_metadata(
    chromium_available: bool,
    tmp_path: Path,
) -> None:
    if not chromium_available:
    ...
    metadata = json.loads(result.metadata_path.read_text(encoding="utf-8"))
    assert metadata["publication_name"] == "Manual Publication"
